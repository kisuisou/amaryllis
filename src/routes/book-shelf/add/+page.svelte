<script lang="ts">
    ...
    let isbn = $state("");
    let books: Array<{ [key: string]: any }> = $state([]);

    let isbn_input: HTMLInputElement | undefined = $state();
    let fetch_promise: Promise<void> | undefined = $state();
    let register_promise: Promise<void> | undefined = $state();
...
<div class="w-50 mx-auto">
    <form class="mx-auto">
        ...
        <div class="mb-3 input-group">
            ...
            >
                {#await fetch_promise}
                    <span
                        class="spinner-border spinner-border-sm my-spinner"
                        role="status"
                        aria-hidden="true"
                    ></span>
                {:then _}
    ...
    {#each books as book}
        <Bookcard
            ...
            creator={String(book.Creator)}
            volume={String(book.Volume)}
            publisher={String(book.Publisher)}
            pubyear={Number(book.PubYear)}
            bind:is_read={book.is_read}
            delFunc={deleteBookData}
        />
    {/each}

    {#if books.length != 0}
        <button class="btn my-btn" onclick={register}>
